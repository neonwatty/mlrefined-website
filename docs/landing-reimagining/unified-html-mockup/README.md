...
- The mockup folder exposes `targets/` as a symlink to the campaign worktree's target assets so both `file://` use and `http://127.0.0.1:8765/index.html` mockup-folder serving can resolve the existing Limner/book assets.
...
- Repo-root server QA with local Playwright at `http://127.0.0.1:8766/docs/landing-reimagining/unified-html-mockup/index.html`: desktop/mobile image checks passed across Home, Notebooks, Chapters, Instructors, and Reviews with no broken visible images; carousel box, stage, and copy heights stayed constant across multiple slides; desktop/mobile overflow checks passed; console/page errors `[]`.
- Mockup-root server QA with local Playwright and Codex Chrome at `http://127.0.0.1:8765/index.html`: desktop/mobile image checks passed across Home, Notebooks, Chapters, Instructors, and Reviews with no broken visible images; Codex Chrome verified Notebooks visible images `broken: []` and Home book cover natural dimensions loaded; carousel box, stage, and copy heights stayed constant across multiple slides; desktop/mobile overflow checks passed; console/page errors `[]`.
- Final polish QA with local Playwright at `http://127.0.0.1:8765/index.html`: hero lead, primary/secondary CTA labels, near-hero trust line, and visual-funnel heading render; mobile nav wraps with `Reviews` visible; carousel pause/resume control toggles `aria-pressed`, manual slide selection pauses autoplay, explicit resume restarts autoplay, active thumbnail exposes `aria-current`/`aria-pressed`, carousel dimensions remain fixed across slides, mobile controls meet 44px touch target checks, no broken visible images, no horizontal overflow, and console/page errors `[]`.
- Stable carousel-shell QA with local Playwright at `http://127.0.0.1:8765/index.html`: slide changes update only image/text/data attributes while preserving the same spotlight container, image node, copy panel, controls, thumbnail rail, and pause button DOM nodes; image/copy transition classes still apply, popover targets stay synchronized, dimensions remain fixed, pause/resume behavior passes, no broken visible images, and console/page errors `[]`.
